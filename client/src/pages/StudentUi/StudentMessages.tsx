...
const StudentMessages: React.FC = () => {
  ...
  /** Grow composer with content; no inner scrollbars (page scrolls like Messenger). */
          ...
          {loading ? (
            ...
            messages.map((m, index) => {
              ...
              return (
                ...
                >
                  <div className={`min-w-0 max-w-[90%] sm:max-w-[75%] flex items-center gap-2 ${isMe ? "justify-end" : ""}`}>
                    {isMe && (
                      <button
                        onClick={() =>
                          setReplyTo({
                    ...
                    <div className={`group relative flex min-w-0 max-w-full flex-col ${isMe ? "items-end" : "items-start"}`}>
                      {!isMe && (
          ...
          <div className="flex items-center gap-3">
            <div className="flex-1 rounded-2xl border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-[#19191c] px-3 py-2 focus-within:ring-2 focus-within:ring-blue-500 focus-within:border-transparent transition-all">
              ...
              <textarea
                ...
                autoComplete="off"
                wrap="soft"
                className="min-h-[2.75rem] min-w-0 flex-1 resize-none overflow-x-hidden whitespace-pre-wrap px-2 py-1.5 bg-transparent text-gray-900 dark:text-white placeholder:text-gray-400 border-0 outline-none focus:outline-none [overflow-wrap:anywhere]"
              />
              <button
                onClick={handleSend}
                disabled={sending || !newMessage.trim()}
                className="h-9 w-9 shrink-0 rounded-full bg-blue-600 hover:bg-blue-700 text-white disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center"
