...
interface StudentWeeklyReportProps {
  /** When true, shows Submit button and back navigation (used from Documents tab) */
  documentMode?: boolean;
  /** Called when document is successfully submitted */
  onSubmitted?: () => void;
  /** Called to navigate back */
  onBack?: () => void;
}

const StudentWeeklyReport: React.FC<StudentWeeklyReportProps> = ({
  documentMode = false,
  onSubmitted,
  onBack,
}) => {
  ...
  const handleSubmitAsDocument = async () => {
    // First save the data, then generate the PDF document
    try {
      setSubmittingDoc(true);

      // Validate at least one week has content
      const hasContent = weeks.some(w => w.tasksAccomplished.trim() || w.knowledgeSkillsValues.trim());
      if (!hasContent) {
        toast.error("Please fill out at least one week before submitting.");
        return;
      }
      ...
      // Then auto-generate the PDF document
      await documentService.autoGenerateDocument('WEEKLY_REPORTS');
      toast.success("Weekly report submitted as document successfully!");

      if (onSubmitted) {
        onSubmitted();
      }
    } catch (error: any) {
      console.error("Error submitting weekly report as document:", error);
      toast.error(error.response?.data?.message || error.message || "Failed to submit weekly report");
    } finally {
      ...
      <div className="bg-white dark:bg-[#212124] rounded-xl p-4 sm:p-6 shadow-sm border border-gray-200 dark:border-gray-700">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center gap-3 flex-1">
            ...
            <h2 className="text-lg sm:text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
              <FileText className="w-5 h-5 sm:w-6 sm:h-6 flex-shrink-0" />
                <span className="line-clamp-2 sm:line-clamp-1">Practicum/Internship Weekly Reports (Form FM-AA-INT-16)</span>
            </h2>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            {documentMode && (
              <button
                onClick={handleSubmitAsDocument}
                disabled={submittingDoc || saving}
                className="flex-1 sm:flex-none flex items-center justify-center space-x-1.5 sm:space-x-2 px-3 sm:px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors text-sm"
              >
                {submittingDoc ? (
                  <Loader2 className="w-4 h-4 animate-spin" />
                ) : (
                ...
                <span>{submittingDoc ? "Submitting..." : "Submit"}</span>
              </button>
            )}
          </div>
        </div>
          ...
          >
            ...
            <div className="mb-4">
              ...
              <p className="text-xs text-gray-400 dark:text-gray-500 mb-2">
                Each line (press Enter) will appear as a bullet point in the generated PDF.
              </p>
              <textarea
                rows={4}
                placeholder={"e.g.\nUI/UX Design\nFront-End of Personal Data Sheet Web Page\nInstalled Laravel, PHP, composer, MySQL"}
